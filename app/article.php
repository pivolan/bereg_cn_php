<?php
	class app_article extends app_mysql{
		public $id,$name,$icon,$text,$title,$url,$date,$date2,$cat,$subcat,$parent;
		protected $table="Article";
		function init(){
			if($article = func_get_args()) $this->sql("SELECT * FROM $this->table WHERE id='$article[0]'");
			else $this->sql("SELECT * FROM $this->table ORDER BY id DESC");
			$this->reset1();
		}
		function initcat($cat){
			$this->sql("SELECT * FROM $this->table WHERE subcat='$cat'");
		}
		...
		function create_db(){
			...
			$this->sql("
				CREATE TABLE $this->table (
				  ...
				  text text NOT NULL,title char(255) NOT NULL,category char(255) NOT NULL,
				  subcat char(255) NOT NULL, parent char(255) NOT NULL,
				  PRIMARY KEY (id)
				)");
		}
