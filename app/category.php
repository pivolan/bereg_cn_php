<?php
	class app_category extends app_mysql{
		...
		function add($POST){
			...
			$this->sql("INSERT INTO $this->table (date,title,text,name, active, parent, icon) VALUES ('$date',
			'$POST[title]','$POST[text]','$POST[name]','$POST[active]','$POST[parent]', '$POST[icon]')");
			return true;
		}
		function update($POST){
			...
			$POST['title']=strip_tags($POST['title'],"<a><p><img><span><div><i><u><b><strong><em><sup><sub><blockquote><table><tr><td><th><tbody><br><ol><li><ul>");
			if(empty($POST['name']))$POST['name']=app_common::ruslat($POST['title']);
			$POST['text']=strip_tags($POST['text'],"<a><p><img><span><div><i><u><b><strong><em><sup><sub><blockquote><table><tr><td><th><tbody>");
