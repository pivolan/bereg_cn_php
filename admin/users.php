<?php
	...
	if($_GET['edit']!=''){
		...
		echo "<form name=\"edit\" action=\"?a=users&act=edit\" method=\"post\">
					...
					статус:<input name=\"status\" type=\"text\" value=\"$line[status]\"><br />
					email:<input name=\"email\" type=\"text\" value=\"$line[email]\"><br />
					adress:<input name=\"adress\" type=\"text\" value=\"$line[adress]\"><br />
					company:<input name=\"company\" type=\"text\" value=\"$line[company]\"><br />
					Телефон:<input name=\"tel\" type=\"text\" value=\"$line[tel]\"><br />
	...
	if($_GET['act']=='edit'){		$_POST['pass']=md5($_POST['pass']);
		...
		login='$_POST[login]', pass='$_POST[pass]', status='$_POST[status]' WHERE id='$_POST[id]'";
		mysql_query($sql);
	}
